...
class _SaveServerKeyPageState extends State<SaveServerKeyPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Save Server Key'), centerTitle: true),
      ...
      body: Column(
        children: [
          ...
          ElevatedButton(
            onPressed: () {
              AuthLocalDatasource().saveMidtransServerKey(
                serverKeyController!.text,
              );
              ScaffoldMessenger.of(
                context,
              ).showSnackBar(const SnackBar(content: Text('Server Key saved')));
